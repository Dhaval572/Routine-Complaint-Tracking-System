...
if (isset($_GET['complaint_id'])) {
  ...
  $sql = "SELECT * FROM complaints WHERE id = '$complaint_id' AND dept_head_id = '$dept_head_id'";
  ...
  if (isset($_POST['solve'])) {
    ...
    $update_sql = "UPDATE complaints SET status = 'solved', response = '$response', remarks = '$remarks' WHERE id = '$complaint_id'";
    if ($conn->query($update_sql)) {
      // Log activity: Complaint Solved by Dept Head
      $activity_sql = "INSERT INTO complaint_activity (complaint_id, activity, activity_by) VALUES ('$complaint_id', 'Complaint Solved by Dept Head', '$dept_head_id')";
...
<head>
  <meta charset="UTF-8">
  <title>Solve Complaint - Dept Head</title>
  ...
  <div class="container mt-5">
    ...
    <?php if (isset($complaint)): ?>
      ...
      <form method="POST" action="">
        ...
        <div class="form-group">
          <label>Remarks</label>
          <textarea name="remarks" required class="form-control" placeholder="Enter your remarks"></textarea>
        </div>
    ...
    <?php else: ?>
      ...
      <?php
      // Use TRIM and LOWER to avoid issues with extra spaces or case differences.
      $list_sql = "SELECT * FROM complaints 
                   WHERE dept_head_id = '$dept_head_id' 
                     AND TRIM(LOWER(status)) != 'solved' 
      ...
      <?php } else {
        echo "<div class='alert alert-info'>No complaints pending solution.</div>";
      } ?>
    <?php endif; ?>
  </div>
</body>
